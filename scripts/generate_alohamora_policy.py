...
def test_website(
    ...
    with open(
        get_results_fname(experiment_name, results_dir, bandwidth, cpu_slowdown, latency) + ".stdut", "ab+"
    ) as outf:
        with open(
            ...
            monitor_process(
                [
                    ...
                    str(reward_func),
                    "--output_file",
                    str(get_results_fname(experiment_name, results_dir, bandwidth, cpu_slowdown, latency) + ".json")
                ],
                60 * 10,
                outf,
                errf,
